...
EMU_DRVS ?= dtm,EtherCat,koganei,lebo_gpio,plc,SFC8000,TEC,DrvRobot,keyence
CTF_RPATH ?=
...
build:
	@$(CMAKE) -S . -B $(BUILD_DIR) -DCMAKE_BUILD_TYPE=$(CMAKE_BUILD_TYPE) -DCMAKE_INSTALL_PREFIX=$(CMAKE_INSTALL_PREFIX) -DCTF_RPATH=$(CTF_RPATH) -DEMU_DRVS="$(EMU_DRVS)"
